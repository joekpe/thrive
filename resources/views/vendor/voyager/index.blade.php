            ...
            <div class="container">
                <div class="row">
                    ...
                    <div class="card-counter primary">
                        ...
                        <span class="count-numbers">{{ count(\App\Models\User::where('role_id', 3)->get() )}}</span>
                        <span class="count-name">Authors</span>
                    </div>
                    </div>
                
                    <div class="col-md-4">
                    <div class="card-counter danger">
                        <i class="icon voyager-book"></i>
                        <span class="count-numbers">{{ count(\App\Models\Book::all() )}}</span>
            ...
            <div class="container">
                <div class="row">
                    ...
                    <div class="card-counter primary">
                        ...
                        <span class="count-numbers">{{ count(\App\Models\Order::where('author_id', Auth::user()->id)->groupBy('invoice_number')->get() )}}</span>
                    ...
                    <div class="card-counter danger">
                        <i class="icon voyager-book"></i>
                        <span class="count-numbers">{{ count(\App\Models\Book::where('user_id', Auth::user()->id)->get() )}}</span>
                        <span class="count-name">Total Books</span>
                    </div>
                    </div>
        ...
        <div class="row">
            @php
                if(Auth::user()->role->name == 'author'){
                    ...
                    $revenue = \App\Models\Balance::select(
                        DB::raw("SUM(amount) AS revenue, DATE_FORMAT(created_at, '%Y-%M') AS date_time")
                    )
                    ->where('user_id', Auth::user()->id)
                    ->where('transaction_type', 'deposit')
                    ->groupBy(DB::raw("DATE_FORMAT(created_at, '%Y-%M')"))
                    ->orderBy(DB::raw("DATE_FORMAT(created_at, '%Y')"), 'desc')->take(12)->get();
                }
                else{
                    $books = \App\Models\Order::select(
                    'book_name', 
                        ...
                        @php
                            if(isset($revenue)){
                                foreach($revenue as $data){
                                    echo "['".$data->date_time."', ".$data->revenue."],";
                                }
                            }
                            // foreach($products as $product) {
                            //     echo "['".$product->name."', ".$product->sales.", ".$product->quantity."],";
